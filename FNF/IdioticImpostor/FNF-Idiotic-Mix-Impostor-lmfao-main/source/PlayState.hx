...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'sussus-moogus':
				dialogue = CoolUtil.coolTextFile(Paths.txt('sussus-moogus/sussus-moogusDialogue'));
			case 'boiling-point':
				dialogue = CoolUtil.coolTextFile(Paths.txt('boiling-point/boiling-pointDialogue'));
		}
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'sussus-moogus' | 'boiling-point':
			    {
					defaultCamZoom = 0.9;

					curStage = 'polus';

					var bg:FlxSprite = new FlxSprite(-600, -300).loadGraphic(Paths.image('impostor_bg'));
					bg.antialiasing = true;
